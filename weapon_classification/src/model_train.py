...
# 导入数据
# 文件的数据中，特征为eq, 类别为label.
def load_data(filepath, input_shape=20):
    df = pd.read_csv(filepath)

    # 标签及词汇表
    labels, vocabulary = list(df['label'].unique()), list(df['eq'].unique())
    ...
    x = [[word_dictionary[word] for word in sent] for sent in df['eq']]
